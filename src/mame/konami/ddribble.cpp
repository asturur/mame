...
void ddribble_state::draw_sprites(bitmap_ind16 &bitmap, const rectangle &cliprect, uint8_t* source, int length, int gfxset, int flipscreen)
{
	...
	while (source < finish)
	{
		...
		int sx = source[3] | ((attr & 0x01) << 8);                // sprite X position (9-bit)
		...
		// The 005885 sprite line buffer is 512 pixels wide and wraps mod-512, but
		// only ~256 columns are scanned out to the screen: the write column
		// (sprite X + pixel offset) is a 9-bit value stored with no on-screen
		// clip, and the read side simply selects the visible slice. A 9-bit X is
		// therefore signed - values 0x100-0x1ff are off-left positions that wrap
		// back in from the left edge. transpen() only clips (it never wraps), so
		// fold X to signed here to reproduce the hardware wrap; otherwise these
		// sprites are drawn off the right edge and clipped away (e.g. the ball
		// vanishes as it re-enters the court from the left after a basket).
		if (sx & 0x100)
			sx -= 0x200;
